...
def run_pipeline(args,i):
    cmd="""python scripts/prep_for_simulations.py --sim-unique-genos {0} 
						...
						--windowsize {5}
						--num-sims {6}
						--sim-snps {7}""".format(str(i),args.bed_files+str(i),args.varbeta_files,args.pathway,args.multiple,args.windowsize,str(int(args.simulation_number)-1),args.sim_snps)
